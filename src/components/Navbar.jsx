  ...
  return (
    <nav className={`navbar${scrolled ? " scrolled" : ""}`}>
      <div className="ef-container">
        <div className="navbar-inner">
          <Link to="/" className="nav-brand">
            {!imgErr ? <img src={LOGO} alt="EduForge" onError={() => setImgErr(true)} /> : <div style={{ width: 40, height: 40, background: "var(--black)", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--font-serif)", fontSize: 18, color: "var(--gold)" }}>E</div>}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <Link to="/start" className="nav-cta" style={{ background: "var(--gold)", color: "var(--black)", fontWeight: 600 }}>Start a Project</Link>
      ...
      <div className={`mobile-menu${open ? " open" : ""}`}>
        ...
        <Link to="/start" className="mobile-cta" style={{ background: "var(--gold)", color: "var(--black)" }}>Start a Project</Link>
      </div>
    </nav>
  );
}
